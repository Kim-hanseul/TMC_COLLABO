import { fetchBoardAPI, writeBoardApi } from "apis/boardApi";
...
//get Saga
function* writeBoardSaga(action : {payload: Article}) {
    const param = action.payload
    try{
        alert(`3. saga 내부 Write 성공 + ${JSON.stringify(param)}`)
        yield call(writeBoardApi, param)
...
export function* watchWriteBoard(){
    yield takeLatest(ArticleActions.writeBoard, writeBoardSaga)
}
